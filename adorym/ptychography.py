...
def reconstruct_ptychography(
    ...
    save_intermediate_level="batch",
    save_history=False,
    store_checkpoint=True,
    use_checkpoint=True,
    force_to_use_checkpoint=False,
    n_batch_per_checkpoint=10,
    save_stdout=False,
    # _____________
    ...
    **kwargs,
    ...
    print_flush(
        "Output folder is {}".format(output_folder), sto_rank, rank, **stdout_options
    )
    ...
    print_flush(
        "Data reading: {} s".format(time.time() - t0), sto_rank, rank, **stdout_options
    )
    print_flush(
        "Data shape: {}".format(original_shape), sto_rank, rank, **stdout_options
    )
    comm.Barrier()
    ...
    if minibatch_size > 1 and common_probe_pos and len(probe_pos) == 1:
        warnings.warn(
            "It seems that you are processing undivided fullfield data with"
    ...
    for ds_level in range(multiscale_level - 1, -1, -1):
        ...
        ds_level = 2**ds_level
        print_flush(
            "Multiscale downsampling level: {}".format(ds_level),
            sto_rank,
            rank,
            **stdout_options,
        )
        ...
        if rank == 0:
            try:
                ...
                print_flush(
                    "Target folder {} exists.".format(output_folder),
                    sto_rank,
                    rank,
                    **stdout_options,
                )
        ...
        # ================================================================================
        voxel_nm = np.array([psize_cm] * 3) * 1.0e7 * ds_level
        lmbda_nm = 1240.0 / energy_ev
        delta_nm = voxel_nm[-1]
        h = get_kernel(
            delta_nm * binning,
            lmbda_nm,
        ...
        if precalculate_rotation_coords:
            if not os.path.exists(
                ...
                comm.Barrier()
                print_flush(
                    "Saving rotation coordinates...", sto_rank, rank, **stdout_options
                )
            ...
            if optimizer == "adam":
                ...
                if distribution_mode is not None or backend != "autograd":
                    raise NotImplementedError(
                        "ScipyOptimizer supports only data parallelism and Autograd backend."
                    )
                ...
                raise ValueError(
                    'Invalid optimizer type. Must be "gd" or "adam" or "cg" or "scipy".'
                )
        opt.create_container(
            [*this_obj_size, 2], use_checkpoint, device_obj, use_numpy=True
        )
        ...
        if use_checkpoint:
            try:
                optimizable_params = load_params_checkpoint(
            ...
            if distribution_mode == "shared_file":
                try:
                    starting_epoch, starting_batch = restore_checkpoint(
                        output_folder, distribution_mode
                    )
                except:
                ...
                try:
                    starting_epoch, starting_batch, obj_arr = restore_checkpoint(
                        output_folder, distribution_mode, opt, dtype=cache_dtype
                    )
                except:
        ...
        if distribution_mode == "shared_file":
            ...
            if needs_initialize:
                print_flush(
                    "Initializing object function in file...",
                    sto_rank,
                    rank,
                    **stdout_options,
                )
                obj.initialize_file_object(
                    save_stdout=save_stdout,
                    timestr=timestr,
            ...
            if needs_initialize:
                print_flush(
                    "Initializing object array...", sto_rank, rank, **stdout_options
                )
                obj.initialize_distributed_array(
                    save_stdout=save_stdout,
                    timestr=timestr,
                    not_first_level=not_first_level,
                    initial_guess=initial_guess,
        ...
        if forward_model == "auto":
            if is_multi_dist:
                ...
                common_probe_pos
                and minibatch_size == 1
                and len(probe_pos) == 1
                and np.allclose(probe_pos[0], 0)
            ):
            ...
            print_flush(
                "Auto-selected forward model: {}.".format(type(forward_model).__name__),
                sto_rank,
                rank,
                **stdout_options,
            )
            ...
            print_flush(
                "Specified forward model: {}.".format(type(forward_model).__name__),
                sto_rank,
                rank,
                **stdout_options,
            )
        ...
        if regularizers is None:
            ...
            if alpha_d not in [0, None]:
                if reweighted_l1:
                    regularizers.append(
                        ReweightedL1Regularizer(
                            alpha_d, alpha_b, unknown_type=unknown_type
                        )
                    )
                else:
                    regularizers.append(
                        L1Regularizer(alpha_d, alpha_b, unknown_type=unknown_type)
                    )
        ...
        if distribution_mode == "shared_file":
            ...
            gradient.initialize_array_with_values(
                np.zeros(this_obj_size), np.zeros(this_obj_size), device=device_obj
            )
        ...
        if beamstop is not None:
            beamstop = w.create_variable(
                beamstop, device=device_obj, requires_grad=False
            )
        ...
        if rank == 0:
            ...
            probe_real_init, probe_imag_init = initialize_probe(
                probe_size,
                probe_type,
            ...
            if n_probe_modes == 1:
                if len(probe_real_init.shape) == 3:
                    if len(probe_real_init) > n_probe_modes:
                        ...
                        print_flush(
                            "Supplied probe mode number is larger than specified. Only the first {} "
                            "are used.".format(n_probe_modes),
                            0,
                            rank,
                        )
                else:
                ...
                if len(probe_real_init.shape) == 3:
                    ...
                    for i_mode in range(n_probe_modes):
                        probe_real.append(
                            np.random.normal(
                                probe_real_init, abs(probe_real_init) * 0.2
                            )
                        )
                        probe_imag.append(
                            np.random.normal(
                                probe_imag_init, abs(probe_imag_init) * 0.2
                            )
                        )
                    ...
                    raise RuntimeError(
                        "Length of supplied supplied probe does not match number of probe modes."
                    )
            if not shared_probe_among_angles:
                probe_real = np.tile(
                    probe_real, [n_theta] + [1] * len(probe_real.shape)
                )
                probe_imag = np.tile(
                    probe_imag, [n_theta] + [1] * len(probe_imag.shape)
                )
        else:
            probe_real = None
            probe_imag = None
        ...
        if common_probe_pos:
            ...
            probe_pos_int_ls = [
                np.round(probe_pos).astype(int) for probe_pos in probe_pos_ls
            ]
        ...
        if is_multi_dist:
            ...
            if not common_probe_pos:
                ...
                for j, (probe_pos, probe_pos_int) in enumerate(
                    zip(probe_pos_ls, probe_pos_int_ls)
                ):
                    probe_pos_correction[j, : len(probe_pos)] = (
                        probe_pos - probe_pos_int
                    )
            n_dists = 1
        prj_affine_ls = np.array([[1.0, 0, 0], [0, 1.0, 0]]).reshape([1, 2, 3])
        ...
        if optimizable_params is None:
            ...
            optimizable_params["probe_pos_offset"] = w.zeros(
                [n_theta, 2], requires_grad=True, device=device_obj
            )
            optimizable_params["prj_pos_offset"] = w.zeros(
                [n_theta, 2], requires_grad=True, device=device_obj
            )
            ...
            if is_sparse_multislice:
                optimizable_params["slice_pos_cm_ls"] = w.create_variable(
                    slice_pos_cm_ls, requires_grad=optimize_slice_pos, device=device_obj
                )

            if is_multi_dist:
                if optimize_free_prop:
                    optimizable_params["free_prop_cm"] = w.create_variable(
                        free_prop_cm,
            ...
            if optimize_tilt:
                optimizable_params["tilt_ls"] = w.create_variable(
                    tilt_ls, device=device_obj, requires_grad=True
                )
            elif initial_tilt is not None:
                tilt_ls = w.create_variable(
                    tilt_ls, device=device_obj, requires_grad=False
                )

            optimizable_params["prj_affine_ls"] = w.create_variable(
                prj_affine_ls, device=device_obj, requires_grad=optimize_prj_affine
            )

            if optimize_ctf_lg_kappa:
                optimizable_params["ctf_lg_kappa"] = w.create_variable(
                    [ctf_lg_kappa],
                    requires_grad=True,
                    device=device_obj,
                    dtype="float64",
        ...
        opt_ls, opt_args_ls = create_and_initialize_parameter_optimizers(
            optimizable_params, locals()
        )
        ...
        if use_epie:
            print_flush(
                "WARNING: Reconstructing using ePIE!", sto_rank, rank, **stdout_options
            )
            warnings.warn(
                "use_epie is True. I will reconstruct using ePIE instead of AD!"
            )
            time.sleep(0.5)
            alt_reconstruction_epie(
                obj.delta,
                obj.beta,
                probe_real,
                probe_imag,
        ...
        comm.Barrier()
        f_conv = open(
            os.path.join(output_folder, "convergence", "loss_rank_{}.txt".format(rank)),
            "w",
        )
        ...
        while cont:
            ...
            t00 = time.time()
            print_flush(
                "Allocating jobs over threads...", sto_rank, rank, **stdout_options
            )
            ...
            for i, i_theta in enumerate(theta_ind_ls):
                ...
                # ================================================================================
                if (
                    distribution_mode is None and update_scheme == "immediate"
                ) and n_pos % minibatch_size != 0:
                    spots_ls = np.append(
                        spots_ls,
                        np.random.choice(
                    ...
                    distribution_mode is not None or update_scheme == "per angle"
                ) and n_pos % n_tot_per_batch != 0:
                    spots_ls = np.append(
                        spots_ls,
                        np.random.choice(
                            spots_ls[: -n_pos % n_tot_per_batch],
                ...
                if common_probe_pos:
                    ...
                    if i == 0:
                        ind_list_rand = np.zeros(
                            [len(theta_ind_ls) * len(spots_ls), 2], dtype="int32"
                        )
                        temp = np.stack(
                            [np.array([i_theta] * len(spots_ls)), spots_ls], axis=1
                        )
                        ...
                        ind_list_rand[
                            i * len(spots_ls) : (i + 1) * len(spots_ls), :
                        ] = temp
                else:
                    if i == 0:
                        ind_list_rand = np.stack(
                            [np.array([i_theta] * len(spots_ls)), spots_ls], axis=1
                        )
                    else:
                        temp = np.stack(
            ...
            print_flush(
                "Allocation done in {} s.".format(time.time() - t00),
                sto_rank,
                rank,
                **stdout_options,
            )
            ...
            for i_batch in range(starting_batch, n_batch):
                ...
                if t_max_min is not None and t_elapsed >= t_max_min:
                    print_flush(
                        "Terminating program because maximum time limit is reached.",
                        sto_rank,
                        rank,
                        **stdout_options,
                    )
                ...
                # ================================================================================
                print_flush(
                    "Epoch {}, batch {} of {} started.".format(
                        i_epoch, i_batch, n_batch
                    ),
                    sto_rank,
                    rank,
                    **stdout_options,
                ...
                if store_checkpoint and i_batch % n_batch_per_checkpoint == 0:
                    if distribution_mode == "shared_file":
                        ...
                        if (distribution_mode is None and rank == 0) or (
                            distribution_mode is not None
                        ):
                            ...
                            save_params_checkpoint(
                                os.path.join(cp_path, "params_{}".format(rank)),
                                optimizable_params,
                            )
                comm.Barrier()
                ...
                this_ind_batch = np.sort(
                    this_ind_batch_allranks[
                        rank * minibatch_size : (rank + 1) * minibatch_size, 1
                    ]
                )
                probe_pos_int = (
                    probe_pos_int
                    if common_probe_pos
                    else probe_pos_int_ls[this_i_theta]
                )
                this_pos_batch = probe_pos_int[this_ind_batch]
                is_last_batch_of_this_theta = (
                    i_batch == n_batch - 1
                    or ind_list_rand[i_batch + 1][0, 0] != this_i_theta
                )
                comm.Barrier()
                print_flush(
                    "  Current rank is processing angle ID {}.".format(this_i_theta),
                    sto_rank,
                    rank,
                    **stdout_options,
                )
                ...
                if (not (distribution_mode is None and not rotate_out_of_loop)) and (
                    ...
                    print_flush(
                        "  Rotating dataset...", sto_rank, rank, **stdout_options
                    )
                    t_rot_0 = time.time()
                    if precalculate_rotation_coords:
                        coord_ls = read_origin_coords(
                            "arrsize_{}_{}_{}_ntheta_{}".format(
                                *this_obj_size, n_theta
                            ),
                            theta_ls[this_i_theta],
                            reverse=False,
                        )
                    else:
                        coord_ls = theta_ls[this_i_theta]
                    if distribution_mode == "shared_file":
                        obj.rotate_data_in_file(
                            coord_ls,
                ...
                if distribution_mode:
                    ...
                    if distribution_mode == "shared_file":
                        if subdiv_probe:
                            obj_rot = obj.read_chunks_from_file(
                                this_pos_batch - np.array([safe_zone_width] * 2),
                        ...
                        if subdiv_probe:
                            obj_rot = obj.read_chunks_from_distributed_object(
                                ...
                                dtype=cache_dtype,
                                n_split=n_split_mpi_ata,
                            )
                            if isinstance(opt, CurveballOptimizer):
                                opt.z_chunk = opt.read_chunks_from_distributed_object(
                                    probe_pos_int - np.array([safe_zone_width] * 2),
                                    this_ind_batch_allranks,
                                    minibatch_size,
                                    subprobe_size + np.array([safe_zone_width] * 2) * 2,
                    ...
                    comm.Barrier()
                    print_flush(
                        "  Chunk reading done in {} s.".format(time.time() - t_read_0),
                        sto_rank,
                        rank,
                        **stdout_options,
                    )
                ...
                if reweighted_l1:
                    with w.no_grad():
                        if distribution_mode:
                            weight_l1 = w.max(obj.chunks) / (
                                w.abs(obj.chunks) + 1e-4 * w.mean(obj.chunks)
                            )
                        else:
                            if i_batch % 10 == 0:
                                weight_l1 = w.max(obj.arr) / (
                                    w.abs(obj.arr) + 1e-4 * w.mean(obj.arr)
                                )
                ...
                if isinstance(opt, CurveballOptimizer):
                    diff.get_l_h_hessian_and_h_x_jacobian_mvps(
                        forward_model, 0, **grad_func_args
                    )
                ...
                comm.Barrier()
                print_flush(
                    "  Gradient calculation done in {} s.".format(
                        time.time() - t_grad_0
                    ),
                    sto_rank,
                    rank,
                ...
                if distribution_mode == "shared_file":
                    ...
                    gradient.write_chunks_to_file(
                        this_pos_batch,
                        *w.split_channel(obj_grads),
                        probe_size,
                        write_difference=False,
                        dtype=cache_dtype,
                    )
                    print_flush(
                        "  Gradient writing done in {} s.".format(
                            time.time() - t_grad_write_0
                        ),
                        0,
                        rank,
                        **stdout_options,
                    ...
                    t_grad_write_0 = time.time()
                    gradient.sync_chunks_to_distributed_object(
                        obj_grads,
                        probe_pos_int,
                        this_ind_batch_allranks,
                        minibatch_size,
                    ...
                    comm.Barrier()
                    print_flush(
                        "  Gradient syncing done in {} s.".format(
                            time.time() - t_grad_write_0
                        ),
                        0,
                        rank,
                    ...
                    if initialize_gradients:
                        del gradient.arr
                        gradient.arr = w.zeros(
                            grads[0].shape, requires_grad=False, device=device_obj
                        )
                ...
                if rank == 0 and debug:
                    print_flush(
                        "  Average gradient is {} for rank 0.".format(w.mean(grads[0])),
                        0,
                        rank,
                        **stdout_options,
                    )
                ...
                if distribution_mode is None:
                    ...
                    shared_file_update_flag = False
                    if (
                        dist_mode_n_batch_per_update is None
                        and not is_last_batch_of_this_theta
                    ):
                        continue
                    elif (
                        dist_mode_n_batch_per_update is not None
                        and i_batch > 0
                        and i_batch % dist_mode_n_batch_per_update != 0
                    ):
                        continue
                    else:
                        initialize_gradients = True
                ...
                with w.no_grad():
                    if distribution_mode is None and optimize_object:
                        if isinstance(opt, ScipyOptimizer):
                            ...
                            obj.arr = opt.apply_gradient(
                                obj.arr, gradient, i_opt_batch, **opt.options_dict
                            )
                ...
                with w.no_grad():
                    ...
                    if update_using_external_algorithm is not None:
                        obj.update_using_external_algorithm(
                            update_using_external_algorithm, locals(), device_obj
                        )
                if distribution_mode is None:
                    w.reattach(obj.arr)
                ...
                # ================================================================================
                optimizable_params = update_parameters(
                    opt_ls, optimizable_params, locals()
                )
                ...
                if distribution_mode and shared_file_update_flag:
                    if precalculate_rotation_coords:
                        ...
                        coord_new = -theta_ls[this_i_theta]
                    print_flush(
                        "  Rotating gradient dataset back...",
                        sto_rank,
                        rank,
                        **stdout_options,
                    )
                    ...
                    comm.Barrier()
                    print_flush(
                        "  Gradient rotation done in {} s.".format(
                            time.time() - t_rot_0
                        ),
                        sto_rank,
                        rank,
                    ...
                    if distribution_mode == "shared_file" and optimize_object:
                        opt.apply_gradient_to_file(
                            obj, gradient, i_batch=i_opt_batch, **optimizer_options_obj
                        )
                        gradient.initialize_gradient_file()
                    elif (
                        distribution_mode == "distributed_object"
                        and obj.arr is not None
                        and optimize_object
                    ):
                        obj.arr = opt.apply_gradient(
                            obj.arr,
                    ...
                    comm.Barrier()
                    print_flush(
                        "  Object update done in {} s.".format(
                            time.time() - t_apply_grad_0
                        ),
                        sto_rank,
                        rank,
                        **stdout_options,
                    ...
                    gc.collect()
                    print_flush(
                        "  Invadidating obj.arr and garbage collection done in {}".format(
                            time.time() - t0_nonify
                        ),
                        sto_rank,
                        rank,
                ...
                if mask is not None:
                    if (
                        ...
                        or distribution_mode == "distributed_object"
                    ):
                        obj.apply_finite_support_mask_to_array(
                            mask, unknown_type=unknown_type, device=device_obj
                        )
                    elif distribution_mode == "shared_file":
                        obj.apply_finite_support_mask_to_file(
                            mask, unknown_type=unknown_type, device=device_obj
                        )
                ...
                if save_intermediate and (
                    (save_intermediate_level == "epoch" and i_batch == n_batch - 1)
                    or save_intermediate_level == "batch"
                ):
                    create_directory_multirank(
                    ...
                    if (
                        distribution_mode != "distributed_object"
                        and rank == 0
                        and is_last_batch_of_this_theta
                    ):
                        output_object(
                            obj,
                            distribution_mode,
                ...
                print_flush(
                    "Minibatch/angle done in {} s; loss (rank 0) is {}.".format(
                        time.time() - t00, current_loss
                    ),
                    sto_rank,
                    rank,
                    **stdout_options,
                ...
                if not cpu_only:
                    print_flush(
                        "GPU memory usage (current/peak): {:.2f}/{:.2f} MB; cache space: {:.2f} MB.".format(
                            w.get_gpu_memory_usage_mb(),
                            w.get_peak_gpu_memory_usage_mb(),
                            w.get_gpu_memory_cache_mb(),
                        ),
                        sto_rank,
                        rank,
                        **stdout_options,
                ...
                f_conv.write(
                    "{},{},{},{}\n".format(
                        i_epoch, i_batch, current_loss, time.time() - t_zero
                    )
                )
                f_conv.flush()
                ...
                if optimizer_batch_number_increment == "angle":
                    if (
                        i_batch == n_batch - 1
                        or ind_list_rand[i_batch + 1][0, 0] != current_i_theta
                    ):
                        i_opt_batch += 1
            ...
            print_flush(
                "Epoch {} (rank {}); Delta-t = {} s; current time = {} s,".format(
                    i_epoch, rank, time.time() - t0, time.time() - t_zero
                ),
                sto_rank,
                rank,
                **stdout_options,
