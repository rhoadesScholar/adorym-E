...
def simulate_ptychography(
    ...
    probe_prj_offset_learning_rate=1e-2,
    ...
    **kwargs,
    ...
    try:
        prj = f.create_group("exchange").create_dataset(
            "data", shape=[n_theta, n_pos, *probe_size], dtype=np.complex64
        )
    except:
        prj = f["exchange/data"]
    ...
    ds_level = 1
    is_multi_dist = (
        True if free_prop_cm not in [None, "inf"] and len(free_prop_cm) > 1 else False
    )
    ...
    # ================================================================================
    voxel_nm = np.array([psize_cm] * 3) * 1.0e7
    ...
    if distribution_mode == "shared_file":
        ...
        print_flush(
            "Initializing object function in file...", sto_rank, rank, **stdout_options
        )
    ...
    if rank == 0:
        ...
        if n_probe_modes == 1:
            ...
            if (
                len(probe_real_init.shape) == 3
                and len(probe_real_init) == n_probe_modes
            ):
                ...
                for i_mode in range(n_probe_modes):
                    probe_real.append(
                        np.random.normal(probe_real_init, abs(probe_real_init) * 0.2)
                    )
                    probe_imag.append(
                        np.random.normal(probe_imag_init, abs(probe_imag_init) * 0.2)
                    )
    ...
    if is_multi_dist:
        if optimize_free_prop:
            optimizable_params["free_prop_cm"] = w.create_variable(
                free_prop_cm, requires_grad=optimize_free_prop, device=device_obj
            )
    ...
    if optimize_ctf_lg_kappa:
        optimizable_params["ctf_lg_kappa"] = w.create_variable(
            [ctf_lg_kappa], requires_grad=True, device=device_obj, dtype="float64"
        )
    ...
    if use_checkpoint:
        try:
            starting_i_theta = np.loadtxt(
                os.path.join(save_path, "sim_checkpoint_i_theta.txt")
            )[0]
            print_flush(
                "Starting from i_theta {}.".format(starting_i_theta),
                sto_rank,
                rank,
                **stdout_options,
            )
    ...
    for i, i_theta in enumerate(theta_ind_ls[starting_i_theta:]):
        ...
        np.savetxt(
            os.path.join(save_path, "sim_checkpoint_i_theta.txt"), [i_theta], fmt="%d"
        )
    ...
    for i_batch in range(n_batch):
        ...
        # ================================================================================
        print_flush(
            "Batch {} of {} started.".format(i_batch, n_batch),
            sto_rank,
            rank,
            **stdout_options,
        )
        ...
        probe_pos_int = (
            probe_pos_int if common_probe_pos else probe_pos_int_ls[this_i_theta]
        )
        this_pos_batch = probe_pos_int[this_ind_batch]
        is_last_batch_of_this_theta = (
            i_batch == n_batch - 1 or ind_list_rand[i_batch + 1][0, 0] != this_i_theta
        )
        comm.Barrier()
        print_flush(
        ...
        if (not (distribution_mode is None and not rotate_out_of_loop)) and (
            ...
            comm.Barrier()
            print_flush(
                "  Dataset rotation done in {} s.".format(time.time() - t_rot_0),
                sto_rank,
                rank,
                **stdout_options,
            )
        ...
        comm.Barrier()
        print_flush(
            "  Batch simulation calculation done in {} s.".format(
                time.time() - t_grad_0
            ),
            sto_rank,
            rank,
        ...
        # ================================================================================
        if complex_output:
            prj[this_i_theta, this_ind_batch] = np.stack(
                w.to_numpy(this_pred_batch[0])
            ) + 1j * w.to_numpy(np.stack(this_pred_batch[1]))
        else:
        ...
        # ================================================================================
        print_flush(
            "Minibatch/angle done in {} s.".format(time.time() - t00),
            sto_rank,
            rank,
            **stdout_options,
        )
