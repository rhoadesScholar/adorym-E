...
def initialize_object_for_sf(
    ...
    if initial_guess is None:
        ...
        initialize_hdf5_with_gaussian(
            dset,
            rank,
            n_ranks,
            random_guess_means_sigmas[0],
            random_guess_means_sigmas[2],
            random_guess_means_sigmas[1],
        ...
        initialize_hdf5_with_arrays(
            dset, rank, n_ranks, initial_guess[0], initial_guess[1], dtype=dtype
        )
    print_flush(
        "Object HDF5 written.",
        0,
...
def initialize_object_for_do(
    ...
    if slice_catalog[rank] is None:
        ...
        slab_shape = [
            slice_catalog[rank][1] - slice_catalog[rank][0],
            *this_obj_size[1:],
        ]
        if initial_guess is None:
            print_flush(
...
def initialize_probe(
    ...
    **kwargs
    ...
    if probe_type == "gaussian":
        ...
        probe_mag, probe_phase = generate_gaussian_map(
            probe_size, 1, probe_mag_sigma, probe_phase_max, probe_phase_sigma
        )
        ...
        probe_init = create_probe_initial_guess_ptycho(
        ...
        raise ValueError(
            "Invalid wavefront type. Choose from 'plane', 'fixed', 'supplied'."
        )
    ...
    if rescale_intensity:
        ...
        if not kwargs["normalize_fft"]:
            ...
            if sign_convention == 1:
                intensity_target = np.sum(np.mean(np.abs(dat), axis=(0, 1))) / np.prod(
                    probe_real.shape[-2:]
                )
            else:
                intensity_target = np.sum(np.mean(np.abs(dat), axis=(0, 1))) * np.prod(
        ...
        print_flush(
            "Probe magnitude scaling factor is {}.".format(s),
            0,
            rank,
...
def create_probe_initial_guess(
    data_fname, dist_nm, energy_ev, psize_nm, raw_data_type="intensity"
):
...
def create_probe_initial_guess_ptycho(
    data_fname, noise=False, raw_data_type="intensity", beamstop=None, sign_convention=1
):
    ...
    if beamstop is not None:
        ...
        xx, yy = np.meshgrid(
            range(beamstop_mask.shape[1]), range(beamstop_mask.shape[0])
        )
        ...
        gaussian_filler = np.exp(
            ((yy - stop_center_y) ** 2 + (xx - stop_center_x) ** 2) / (-4 * sigma**2)
        )
    ...
    if noise:
        ...
        wavefront += np.random.normal(
            size=wavefront.shape, loc=wavefront_mean, scale=wavefront_mean * 0.2
        )
...
def realign_image_fourier(a_real, a_imag, shift, axes=(0, 1), device=None):
    ...
    freq_x = w.create_variable(
        freq_x, requires_grad=False, device=device, dtype=w.get_dtype(f_real)
    )
    freq_y = w.create_variable(
        freq_y, requires_grad=False, device=device, dtype=w.get_dtype(f_real)
    )
    mult_real, mult_imag = w.exp_complex(
        0.0, -2 * PI * (freq_x * shift[1] + freq_y * shift[0])
    )
    # Reshape for broadcasting
    if len(s) > max(axes) + 1:
        mult_real = w.reshape(
            mult_real, list(mult_real.shape) + [1] * (len(s) - (max(axes) + 1))
        )
        mult_real = w.tile(mult_real, [1, 1] + list(s[max(axes) + 1 :]))
        mult_imag = w.reshape(
            mult_imag, list(mult_imag.shape) + [1] * (len(s) - (max(axes) + 1))
        )
        mult_imag = w.tile(mult_imag, [1, 1] + list(s[max(axes) + 1 :]))
    a_real, a_imag = (
        f_real * mult_real - f_imag * mult_imag,
...
def rescale(arr, scale, device=None, override_backend=None):
    ...
    xx = w.create_variable(
        xx, requires_grad=False, device=device, override_backend=override_backend
    )
    yy = w.create_variable(
        yy, requires_grad=False, device=device, override_backend=override_backend
    )
    coord_old_1 = w.reshape(
        yy / scale + image_center[0],
        [arr_size[0] * arr_size[1]],
        override_backend=override_backend,
    )
    coord_old_2 = w.reshape(
        xx / scale + image_center[1],
        [arr_size[0] * arr_size[1]],
        override_backend=override_backend,
    )
    coord_old_1 = w.clip(
        coord_old_1, 0, arr_size[0] - 1, override_backend=override_backend
    )
    coord_old_2 = w.clip(
        coord_old_2, 0, arr_size[1] - 1, override_backend=override_backend
    )

    coord_old_floor_1 = w.floor_and_cast(
        coord_old_1, dtype="int64", override_backend=override_backend
    )
    coord_old_ceil_1 = w.ceil_and_cast(
        coord_old_1, dtype="int64", override_backend=override_backend
    )
    coord_old_floor_2 = w.floor_and_cast(
        coord_old_2, dtype="int64", override_backend=override_backend
    )
    coord_old_ceil_2 = w.ceil_and_cast(
        coord_old_2, dtype="int64", override_backend=override_backend
    )
    integer_mask_1 = abs(coord_old_ceil_1 - coord_old_floor_1) < 1e-5
    integer_mask_2 = abs(coord_old_ceil_2 - coord_old_floor_2) < 1e-5

    fac_ff = (coord_old_ceil_1 + integer_mask_1 - coord_old_1) * (
        coord_old_ceil_2 + integer_mask_2 - coord_old_2
    )
    fac_fc = (coord_old_ceil_1 + integer_mask_1 - coord_old_1) * (
        coord_old_2 - coord_old_floor_2
    )
    fac_cf = (coord_old_1 - coord_old_floor_1) * (
        coord_old_ceil_2 + integer_mask_2 - coord_old_2
    )
...
def calculate_original_coordinates_for_rotation(
    array_size, coord_new, theta, override_backend=None, device=None
):
    image_center = [(x - 1) / 2 for x in array_size]
    m_rot = w.create_variable(
        [
            [
                w.cos(theta, override_backend=override_backend),
                -w.sin(theta, override_backend=override_backend),
            ],
            [
                w.sin(theta, override_backend=override_backend),
                w.cos(theta, override_backend=override_backend),
...
def rotate_no_grad(
    obj, theta, axis=0, override_backend=None, interpolation="bilinear", device=None
):
    """
    ...
    coord_new = w.create_variable(
        coord_new, device=device, override_backend=override_backend, requires_grad=False
    )
    coord_old = calculate_original_coordinates_for_rotation(
        arr_size, coord_new, theta, override_backend=override_backend
    )
...
def save_rotation_lookup(array_size, theta_ls, dest_folder=None, override_backend=None):
    ...
    for i, theta in enumerate(theta_ls[rank:n_theta:n_ranks]):
        ...
        coord_old = calculate_original_coordinates_for_rotation(
            array_size, coord_new, theta, override_backend=override_backend
        )
        coord_inv = calculate_original_coordinates_for_rotation(
            array_size, coord_new, -theta, override_backend=override_backend
        )
        ...
        np.save(
            os.path.join(dest_folder, "{:.5f}".format(theta)),
            coord_old.astype("float16"),
        )
        np.save(
            os.path.join(dest_folder, "_{:.5f}".format(theta)),
            coord_inv.astype("float16"),
        )
    return None
...
def read_origin_coords(src_folder, theta, reverse=False):
    ...
    if not reverse:
        coords = np.load(
            os.path.join(src_folder, "{:.5f}.npy".format(theta)), allow_pickle=True
        )
    else:
        coords = np.load(
            os.path.join(src_folder, "_{:.5f}.npy".format(theta)), allow_pickle=True
        )
    return coords
...
def apply_rotation(
    ...
    if not reverse:
        try:
            obj_rot = w.grid_sample(
                obj, coord_old, axis=axis, interpolation=interpolation, device=device
            )
        except:
            warnings.warn(
                "PyTorch is not available, so I am applying rotation using apply_rotation_primitive which may "
            ...
            obj_rot = apply_rotation_primitive(
                obj, coord_old, axis=axis, interpolation=interpolation, device=device
            )
    else:
        obj_rot = apply_rotation_transpose(
            obj, coord_old, axis=axis, interpolation=interpolation, device=device
        )
    return obj_rot


def apply_rotation_primitive(
    obj, coord_old, interpolation="bilinear", axis=0, device=None, override_backend=None
):
    ...
    for i in range(len(obj.shape)):
        if i != axis and i <= 2:
            axes_rot.append(i)
    coord_old = w.create_variable(
        coord_old, device=device, requires_grad=False, override_backend=override_backend
    )

    if interpolation == "nearest":
        coord_old_1 = w.round_and_cast(
            coord_old[:, 0], override_backend=override_backend
        )
        coord_old_2 = w.round_and_cast(
            coord_old[:, 1], override_backend=override_backend
        )
    else:
    ...
    coord_old_1 = w.clip(
        coord_old_1, 0, s[axes_rot[0]] - 1, override_backend=override_backend
    )
    coord_old_2 = w.clip(
        coord_old_2, 0, s[axes_rot[1]] - 1, override_backend=override_backend
    )
...
def apply_rotation_to_hdf5(
    ...
    if precalculate_rotation_coords:
        if interpolation == "nearest":
            ...
            fac_ff = (coord_old_ceil_1 - coord_old_1) * (coord_old_ceil_2 - coord_old_2)
            fac_fc = (coord_old_ceil_1 - coord_old_1) * (
                coord_old_2 - coord_old_floor_2
            )
            fac_cf = (coord_old_1 - coord_old_floor_1) * (
                coord_old_ceil_2 - coord_old_2
            )
            fac_cc = (coord_old_1 - coord_old_floor_1) * (
                coord_old_2 - coord_old_floor_2
            )
            if not monochannel:
        ...
        for i_slice in slice_ls:
            obj = dset[i_slice]
            obj_rot = sp_rotate(
                obj, -coord_old, axes=(1, 2), reshape=False, order=1, mode="nearest"
            )
            dset_2[i_slice] = obj_rot
...
def initialize_hdf5_with_gaussian(
    dset,
    rank,
    n_ranks,
    delta_mu,
    ...
    np.random.seed(rank)
    for i_slice in slice_ls:
        slice_delta = np.random.normal(
            size=[s[1], s[2]], loc=delta_mu, scale=delta_sigma
        )
...
def initialize_hdf5_with_arrays(
    dset, rank, n_ranks, init_delta, init_beta, dtype="float32"
):
    ...
    for i_slice in slice_ls:
        ...
        if init_beta is not None:
            slice_data[...] = np.stack(
                [init_delta[i_slice], init_beta[i_slice]], axis=-1
            )
        else:
...
def get_subblocks_from_distributed_object_mpi(
    ...
    if n_split == "auto":
        ...
        n_recipients = min(
            [n_ranks * minibatch_size, ceil(whole_object_size[1] / probe_size[1])]
        )
        n_byte = int(re.findall("\d+", dtype)[0]) / 8
        n_split = ceil(
            (chunk_thickness * chunk_width * chunk_depth * 2 * n_byte * n_recipients)
            / (2**31)
        )
    chunk_batch_ls_ls = [[None] * n_ranks for _ in range(n_split)]

    my_slice_range = slice_catalog[rank]
    my_ind_batch = np.sort(
        this_ind_batch_allranks[rank * minibatch_size : (rank + 1) * minibatch_size, 1]
    )
    ...
    if my_slice_range is not None:
        ...
        for i_rank in range(n_ranks):
            their_ind_batch = np.sort(
                this_ind_batch_allranks[
                    i_rank * minibatch_size : (i_rank + 1) * minibatch_size, 1
                ]
            )
            ...
            for i_pos, their_pos in enumerate(their_pos_batch):
                their_slice_range = [
                    max([their_pos[0], 0]),
                    min([their_pos[0] + probe_size[0], whole_object_size[0]]),
                ]
                if their_slice_range[1] <= my_slice_range[0]:
                    continue
                if (their_slice_range[1] - my_slice_range[0]) * (
                    their_slice_range[0] - my_slice_range[1]
                ) < 0:
                    line_st = (
                        max([my_slice_range[0], their_slice_range[0]])
                        - my_slice_range[0]
                    )
                    line_end = (
                        min([my_slice_range[1], their_slice_range[1]])
                        - my_slice_range[0]
                    )
    ...
    for i_split in range(n_split):
        if debug:
            print_alltoall_data_shape(chunk_batch_ls_ls, i_split=i_split)
    ...
    for i_rank in range(n_ranks):
        if chunk_batch_ls_ls[0][i_rank] is None:
            ...
            for i_pos in range(len(chunk_batch_ls_ls[0][i_rank])):
                temp = [
                    chunk_batch_ls_ls[i_split][i_rank][i_pos]
                    for i_split in range(n_split)
                ]
    ...
    for i_pos, my_pos in enumerate(my_pos_batch):
        ...
        if my_pos[1] >= whole_object_size[1] or my_pos[1] + probe_size[1] <= 0:
            ...
            my_chunk_slice_range = [
                max([my_pos[0], 0]),
                min([my_pos[0] + probe_size[0], whole_object_size[0]]),
            ]
            for i_rank, their_slice_range in enumerate(slice_catalog):
                if their_slice_range is not None:
                    if their_slice_range[1] <= my_chunk_slice_range[0]:
                        continue
                    if (their_slice_range[0] - my_chunk_slice_range[1]) * (
                        their_slice_range[1] - my_chunk_slice_range[0]
                    ) < 0:
...
def sync_subblocks_among_distributed_object_mpi(
    obj,
    my_slab,
    slice_catalog,
    probe_pos,
            ...
            for i_pos, my_pos in enumerate(my_pos_batch):
                ...
                if (their_slice_range[0] - my_chunk_slice_range[1]) * (
                    ...
                    if my_pos[0] + probe_size[0] > their_slice_range[1]:
                        my_chunk = my_chunk[
                            : -(my_pos[0] + probe_size[0] - their_slice_range[1])
                        ]
                    for i_split in range(n_split):
                        step = s[2] // n_split
                        st = step * i_split
        ...
        for i_rank in range(n_ranks):
            ...
            for i_pos, their_pos in enumerate(their_pos_batch):
                ...
                if (their_slice_range[1] - my_slice_range[0]) * (
                    ...
                    if (
                        their_pos[1] >= whole_object_size[1]
                        or their_pos[1] + probe_size[1] <= 0
                    ):
                        ind_pos += 1
                        continue
                    else:
                        ...
                        if their_pos[1] + probe_size[1] > whole_object_size[1]:
                            their_chunk = their_chunk[
                                :,
                                : -(
                                    their_pos[1] + probe_size[1] - whole_object_size[1]
                                ),
                                :,
                                :,
                            ]
                        my_slab[
                            line_st:line_end,
                            max([0, their_pos[1]]) : min(
                                [whole_object_size[1], their_pos[1] + probe_size[1]]
                            ),
                            :,
                            :,
                        ] += their_chunk
...
def get_subblocks_from_distributed_object(
    obj,
    slice_catalog,
    probe_pos,
    this_ind_batch_allranks,
        ...
        for i_rank in range(n_ranks):
            ...
            for i_pos, their_pos in enumerate(their_pos_batch):
                ...
                if (their_slice_range[1] - my_slice_range[0]) * (
                    ...
                    np.save(
                        os.path.join(
                            tmp_folder, "tr{}_ip{}_sr{}.npy".format(i_rank, i_pos, rank)
                        ),
                        my_chunk,
                    )
                ...
                if (their_slice_range[0] - my_chunk_slice_range[1]) * (
                    ...
                    my_chunk.append(
                        np.load(
                            os.path.join(
                                tmp_folder,
                                "tr{}_ip{}_sr{}.npy".format(rank, i_pos, i_rank),
                            )
                        )
                    )
                    i_req += 1
...
def sync_subblocks_among_distributed_object(
    obj,
    slice_catalog,
    probe_pos,
    this_ind_batch_allranks,
        ...
        for i_rank, their_slice_range in enumerate(slice_catalog):
            if their_slice_range is not None:
                ...
                if (their_slice_range[0] - my_chunk_slice_range[1]) * (
                    ...
                    # Pad/trim top-bottom.
                    if my_pos[0] < their_slice_range[0]:
                        my_chunk_send = my_chunk_send[
                            their_slice_range[0] - my_pos[0] :
                        ]
                    if my_pos[0] + probe_size[0] > their_slice_range[1]:
                        my_chunk_send = my_chunk_send[
                            : -(my_pos[0] + probe_size[0] - their_slice_range[1])
    ...
    if my_slice_range is not None:
        my_slab = np.zeros(
            [
                my_slice_range[1] - my_slice_range[0],
                whole_object_size[1],
                whole_object_size[2],
                2,
            ]
        ...
        for i_rank in range(n_ranks):
            ...
            for i_pos, their_pos in enumerate(their_pos_batch):
                ...
                if (their_slice_range[1] - my_slice_range[0]) * (
                    ...
                    their_chunk = np.load(
                        os.path.join(
                            tmp_folder, "tr{}_ip{}_sr{}.npy".format(rank, i_pos, i_rank)
                        )
                    )
                    # Trim left-right.
                    if their_pos[1] < 0:
...
def get_rotated_subblocks(
    ...
    for coords in this_pos_batch:
        ...
        if (
            sum(
                abs(
                    np.array([line_st, line_end, px_st, px_end])
                    - np.array([line_st_clip, line_end_clip, px_st_clip, px_end_clip])
                )
            )
            > 0
        ):
            if not monochannel:
                if unknown_type == "delta_beta":
                    this_block = np.pad(
                        this_block,
                        [
                            [line_st_clip - line_st, line_end - line_end_clip],
    ...
    for i_batch, coords in enumerate(this_pos_batch):
        ...
        if (
            ...
            this_block = this_block[
                line_st_clip
                - line_st : this_block.shape[0]
                - (line_end - line_end_clip),
...
def pad_object(
    ...
    if np.count_nonzero(pad_arr) > 0:
        if unknown_type == "delta_beta":
            ...
            args_2 = {}
            if mode == "constant":
                args_1["constant_values"] = 1
            if mode == "constant":
                args_2["constant_values"] = 0
            obj_rot = w.stack(
                [
                    w.pad(
                        obj_rot[slicer0],
...
def pad_object_edge(
    obj_rot, this_obj_size, probe_pos, probe_size, override_backend=None, pad_arr=None
):
    """
    ...
    """
    if pad_arr is None:
        pad_arr = calculate_pad_len(
            this_obj_size, probe_pos, probe_size, unknown_type="delta_beta"
        )
...
def generate_sphere(shape, radius, anti_aliasing=5):
    ...
    res[
        center_res[0] - int(a.shape[0] / 2) : center_res[0] + int(a.shape[0] / 2),
        center_res[1] - int(a.shape[0] / 2) : center_res[1] + int(a.shape[0] / 2),
        center_res[2] - int(a.shape[0] / 2) : center_res[2] + int(a.shape[0] / 2),
    ] = a
...
def upsample_2x(arr):
    ...
    if arr.ndim == 4:
        out_arr = np.zeros(
            [arr.shape[0] * 2, arr.shape[1] * 2, arr.shape[2] * 2, arr.shape[3]]
        )
...
def multidistance_ctf(
    ...
    dist_nm_ls = dist_cm_ls * 1.0e7
    lmbda_nm = 1.24 / energy_kev
    psize_nm = psize_cm * 1.0e7
    prj_shape = prj_ls.shape[1:]

    u_max = 1.0 / (2.0 * psize_nm)
    v_max = 1.0 / (2.0 * psize_nm)
    ...
    phase = np.sum(
        np.fft.fftshift(np.fft.fft2(prj_ls - 1, axes=(-2, -1)), axes=(-2, -1))
        * (np.sin(xi_ls) + 1.0 / kappa * np.cos(xi_ls)),
        axis=0,
    )
    phase /= (
        np.sum(2 * (np.sin(xi_ls) + 1.0 / kappa * np.cos(xi_ls)) ** 2, axis=0) + alpha
    )
...
def get_block_division(original_grid_shape, n_ranks):
    ...
    n_blocks_y = int(
        np.round(np.sqrt(original_grid_shape[0] / original_grid_shape[1] * n_ranks))
    )
    n_blocks_x = int(
        np.round(np.sqrt(original_grid_shape[1] / original_grid_shape[0] * n_ranks))
    )
    n_blocks = n_blocks_x * n_blocks_y
    block_size = ceil(
        max([original_grid_shape[0] / n_blocks_y, original_grid_shape[1] / n_blocks_x])
    )

    while n_blocks > n_ranks:
        if (
            n_blocks_y * block_size - original_grid_shape[0]
            > n_blocks_x * block_size - original_grid_shape[1]
        ):
            n_blocks_y -= 1
        else:
            n_blocks_x -= 1
...
def get_rotated_subblocks_with_tilt(
    dset,
    this_pos_batch,
    coord_old,
    probe_size,
    ...
    for coords in this_pos_batch:
        ...
        (
            ...
            unique_pos,
            repeats,
        ) = convert_to_hdf5_indexing(ind_old)

        this_block_temp = dset[
            max([0, coord0_vec[0]]) : min([whole_object_size[0], coord0_vec[-1] + 1]),
            sorted_coords_unique,
            :,
        ]
        ...
        if interpolation == "bilinear":
            ...
            integer_mask_1 = (
                abs(np.ceil(ind_old_float_1) - ind_old_float_1) < 1e-5
            ).astype(float)
            integer_mask_2 = (
                abs(np.ceil(ind_old_float_2) - ind_old_float_2) < 1e-5
            ).astype(float)
            if not monochannel:
                for i_chan in range(this_block.shape[2]):
                    this_block[:, :, i_chan] = (
                        this_block_ff[:, :, i_chan]
                        * (
                            (np.ceil(ind_old_float_1) + integer_mask_1)
                            - ind_old_float_1
                        )
                        * (
                            (np.ceil(ind_old_float_2) + integer_mask_2)
                            - ind_old_float_2
                        )
                        + this_block_fc[:, :, i_chan]
                        * (
                            (np.ceil(ind_old_float_1) + integer_mask_1)
                            - ind_old_float_1
                        )
        ...
        # Reshape and pad
        if not monochannel:
            this_block = np.reshape(
                this_block,
                [this_block.shape[0], block_shape[1], whole_object_size[2], 2],
            )
            if coord0_vec[0] < 0:
                this_block = np.pad(
                    this_block,
                    [[-coord0_vec[0], 0], [0, 0], [0, 0], [0, 0]],
                    mode="edge",
                )
            if coord0_vec[-1] + 1 - whole_object_size[0] > 0:
                this_block = np.pad(
                    this_block,
                    [
                        [0, coord0_vec[-1] + 1 - whole_object_size[0]],
                        [0, 0],
                        [0, 0],
                        [0, 0],
                    ],
            ...
            this_block = np.reshape(
                this_block, [this_block.shape[0], block_shape[1], whole_object_size[2]]
            )
            if coord0_vec[0] < 0:
                this_block = np.pad(
                    this_block, [[-coord0_vec[0], 0], [0, 0], [0, 0]], mode="edge"
                )
            if coord0_vec[-1] + 1 - whole_object_size[0] > 0:
                this_block = np.pad(
                    this_block,
...
def write_subblocks_to_file_with_tilt(
    dset,
    this_pos_batch,
    obj_delta,
    obj_beta,
    ...
    for i_batch, coords in enumerate(this_pos_batch):
        ...
        coord_old_clip_mask = (
            (ind_old_1 >= 0)
            * (ind_old_1 <= whole_object_size[1] - 1)
            * (ind_old_2 >= 0)
            * (ind_old_2 <= whole_object_size[2] - 1)
        )
        ...
        # mask
        discont_pos = discont_pos[
            discont_pos < (whole_object_size[1] - 1) * (whole_object_size[2])
        ]
        discont_pos = discont_pos[
            discont_pos % whole_object_size[2] != whole_object_size[2] - 1
        ]
        ...
        obj_crop_bot = min(
            [
                obj_delta.shape[1] - (coord0_vec[-1] + 1 - whole_object_size[0]),
                obj_delta.shape[1],
            ]
        )
        ...
        if interpolation == "bilinear":
            ...
            vals_delta_ff = obj_delta[
                i_batch, obj_crop_top:obj_crop_bot, ind_new_floor_1, ind_new_floor_2
            ].transpose()
            vals_delta_fc = obj_delta[
                i_batch, obj_crop_top:obj_crop_bot, ind_new_floor_1, ind_new_ceil_2
            ].transpose()
            vals_delta_cf = obj_delta[
                i_batch, obj_crop_top:obj_crop_bot, ind_new_ceil_1, ind_new_floor_2
            ].transpose()
            vals_delta_cc = obj_delta[
                i_batch, obj_crop_top:obj_crop_bot, ind_new_ceil_1, ind_new_ceil_2
            ].transpose()
            vals_delta = (
                vals_delta_ff
                * (ind_new_ceil_1 + integer_mask_1 - ind_new_1)
                * (ind_new_ceil_2 + integer_mask_2 - ind_new_2)
                + vals_delta_fc
                * (ind_new_ceil_1 + integer_mask_1 - ind_new_1)
                * (ind_new_2 - ind_new_floor_2)
                + vals_delta_cf
                * (ind_new_1 - ind_new_floor_1)
                * (ind_new_ceil_2 + integer_mask_2 - ind_new_2)
                + vals_delta_cc
                * (ind_new_1 - ind_new_floor_1)
                * (ind_new_2 - ind_new_floor_2)
            ...
            if not monochannel:
                vals_beta_ff = obj_beta[
                    i_batch, obj_crop_top:obj_crop_bot, ind_new_floor_1, ind_new_floor_2
                ].transpose()
                vals_beta_fc = obj_beta[
                    i_batch, obj_crop_top:obj_crop_bot, ind_new_floor_1, ind_new_ceil_2
                ].transpose()
                vals_beta_cf = obj_beta[
                    i_batch, obj_crop_top:obj_crop_bot, ind_new_ceil_1, ind_new_floor_2
                ].transpose()
                vals_beta_cc = obj_beta[
                    i_batch, obj_crop_top:obj_crop_bot, ind_new_ceil_1, ind_new_ceil_2
                ].transpose()
                vals_beta = (
                    vals_beta_ff
        ...
        if not monochannel:
            dset[
                max([0, coord0_vec[0]]) : min(
                    [whole_object_size[0], coord0_vec[-1] + 1]
                ),
                ind_old,
                :,
    ...
    if obj.arr is not None:
        if unknown_type == "delta_beta":
            ...
            if distribution_mode == "distributed_object":
                ...
                fname1 += "_rank_{}".format(rank)
            dxchange.write_tiff(
                np.sqrt(obj0**2 + obj1**2),
                os.path.join(output_folder, fname0),
                dtype="float32",
                overwrite=True,
            )
            dxchange.write_tiff(
                np.arctan2(obj1, obj0),
                os.path.join(output_folder, fname1),
                dtype="float32",
                overwrite=True,
            )
...
def output_probe(
    ...
    dxchange.write_tiff(
        np.sqrt(probe_real**2 + probe_imag**2),
...
def get_subdividing_params(image_shape, n_blocks_y, n_blocks_x, **kwargs):
    ...
    n_blocks = n_blocks_x * n_blocks_y
    block_size_y, block_size_x = np.ceil(
        [image_shape[0] / n_blocks_y, image_shape[1] / n_blocks_x]
    ).astype(int)
    if rank == 0:
...
def subdivide_image(img, block_range_ls, override_backend=None):
    ...
    block_size_sz_y, block_size_sz_x = (
        block_range_ls[0][1] - block_range_ls[0][0],
        block_range_ls[0][3] - block_range_ls[0][2],
    )
    img, pad_arr = pad_object(
        img,
        img.shape,
        block_range_ls[:, 0:3:2],
        [block_size_sz_y, block_size_sz_x],
        mode="edge",
        override_backend=override_backend,
    )
...
def _upsampled_dft(
    data_real, data_imag, upsampled_region_size, upsample_factor=1, axis_offsets=None
):
    """
    ...
    for (n_items, ups_size, ax_offset) in dim_properties[::-1]:
        ffreq = w.create_constant(
            np.fft.fftfreq(n_items, upsample_factor), device=w.get_var_device(data_real)
        )
        a = (w.arange(ups_size) - ax_offset)[:, None]
        a = w.create_constant(a, device=w.get_var_device(data_real))
        kernel = a * ffreq
        kernel_real, kernel_imag = w.exp_complex(0.0, -im2pi_imag * kernel)

        # Equivalent to:
        #   data[i, j, k] = kernel[i, :] @ data[j, k].T
        # data = np.tensordot(kernel, data, axes=(1, -1))
        d_real = w.tensordot(kernel_real, data_real, axes=(1, -1)) - w.tensordot(
            kernel_imag, data_imag, axes=(1, -1)
        )
        d_imag = w.tensordot(kernel_real, data_imag, axes=(1, -1)) + w.tensordot(
            kernel_imag, data_real, axes=(1, -1)
        )
        data_real = d_real
        data_imag = d_imag
...
def phase_correlation(img, ref, upsample_factor=1):
    ...
    prod_real, prod_imag = w.complex_mul(
        f_img_real, f_img_imag, f_ref_real, -f_ref_imag
    )
    cc_real, cc_imag = w.ifft2(prod_real, prod_imag)
    cc = cc_real**2 + cc_imag**2
    shifts = w.argmax(cc)
    shifts = w.create_constant(
        [shifts // img_shape[1], shifts % img_shape[1]], device=w.get_var_device(img)
    )
    ...
    if upsample_factor > 1:
        ...
        upsampled_region_size = w.create_constant(
            [upsampled_region_size, upsampled_region_size], device=w.get_var_device(img)
        )
        ...
        cc_real, cc_imag = _upsampled_dft(
            prod_real,
            -prod_imag,
            upsampled_region_size,
            upsample_factor,
