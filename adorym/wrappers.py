...
if flag_pytorch_avail:
    try:
        pytorch_dtype_query_mapping_dict = {
            tc.float32: "float32",
            tc.float64: "float64",
            "float32": "float32",
            "float64": "float64",
...
@set_bn
def create_variable(
    arr, dtype="float32", device=None, requires_grad=True, backend="autograd"
):
    """
    ...
    if backend == "autograd":
        ...
        if dtype is not None:
            args["dtype"] = getattr(
                engine_dict["pytorch"], dtype_mapping_dict[dtype]["pytorch"]
            )
    ...
    if backend == "autograd":
        ...
        for i, node in enumerate(kwargs_ls):
            if i in opt_args_ls:
                dx_ls.append(node)
        grads = tag.grad(
            l, dx_ls, retain_graph=True, create_graph=False, allow_unused=False
        )
        # grads = []
...
def get_allocated_tensors(backend="autograd"):
    ...
    if backend == "pytorch":
        ...
        for obj in objects:
            try:
                if tc.is_tensor(obj) or (
                    hasattr(obj, "data") and tc.is_tensor(obj.data)
                ):
...
@set_bn
def fft2_and_shift(
    var_real, var_imag, axes=(-2, -1), backend="autograd", normalize=False
):
    norm = None if not normalize else "ortho"
    var = var_real + 1j * var_imag
    if backend == "autograd":
...
@set_bn
def convolve_with_transfer_function(
    arr_real, arr_imag, h_real, h_imag, axes=(-2, -1), backend="autograd"
):
...
@set_bn
def convolve_with_impulse_response(
    arr_real,
    arr_imag,
    h_real,
    h_imag,
    ...
    f_real, f_imag = fft2(
        arr_real, arr_imag, axes=axes, override_backend=backend, normalize=normalize
    )
...
def split_channel(var, backend="autograd"):
    if backend == "autograd":
        ...
        slicer = [slice(None)] * (var.ndim - 1) + [
            0
        ]  # this removes the last singleton dimension...
        return var0[tuple(slicer)], var1[tuple(slicer)]
...
def floor_and_cast(var, dtype="int32", backend="autograd"):
    return cast(
        floor(var, override_backend=backend), dtype=dtype, override_backend=backend
    )
...
def ceil_and_cast(var, dtype="int32", backend="autograd"):
    return cast(
        ceil(var, override_backend=backend), dtype=dtype, override_backend=backend
    )
...
def pad(var, pad_len, mode="constant", constant_values=0, backend="autograd"):
    ...
    args = {}
    mode_dict = {
        "constant": {"autograd": "constant", "pytorch": "constant"},
    ...
    if backend == "autograd":
        ...
        return tc.nn.functional.pad(
            var, pad_len, mode=mode_dict[mode][backend], value=constant_values
        )
    elif backend == "numpy":
...
@set_bn
def grid_sample(
    arr, grid, interpolation="bilinear", axis=0, device=None, backend="autograd"
):
    """
    ...
    if axis != 0:
        ...
        axis_arrangement[0], axis_arrangement[q] = (
            axis_arrangement[q],
            axis_arrangement[0],
        )
    if axis_arrangement[2] > axis_arrangement[3]:
        axis_arrangement[2], axis_arrangement[3] = (
            axis_arrangement[3],
            axis_arrangement[2],
        )
    ...
    arr_shape = create_constant(
        arr.shape[2:],
        dtype=pytorch_dtype_query_mapping_dict[arr.dtype],
        device=get_var_device(arr),
    )
    grid = -1 + 2.0 * grid / arr_shape + 1.0 / arr_shape

    grid = reshape(grid, [1, *arr.shape[2:4], 2], override_backend="pytorch")
    grid = tile(grid, [arr.shape[0], 1, 1, 1], override_backend="pytorch")
    grid = cast(
        grid, pytorch_dtype_query_mapping_dict[arr.dtype], override_backend="pytorch"
    )
    arr = tc.nn.functional.grid_sample(
        arr, grid, padding_mode="border", mode=interpolation, align_corners=False
    )
    arr = permute_axes(
        arr,
        [
            axis_arrangement.index(0),
            axis_arrangement.index(1),
            axis_arrangement.index(2),
            axis_arrangement.index(3),
        ],
        override_backend="pytorch",
    )
    if flag_convert_arr:
        arr = arr.data.numpy()
    return arr
...
def affine_transform(arr, transform, backend="autograd"):
    ...
    if backend == "autograd":
        raise NotImplementedError(
            "Rescaling in Autograd is not yet implemented. Use Pytorch backend instead."
        )
    elif backend == "pytorch":
        n = arr.shape[0]
        ...
        m = cast(
            tile(m, [n, 1, 1], override_backend=backend),
            pytorch_dtype_query_mapping_dict[arr.dtype],
...
def rotate(arr, theta, axis=0, backend="autograd", device=None):
    ...
    if backend == "autograd":
        warnings.warn(
            "Rotate (with grad) in Autograd is not yet implemented. Use Pytorch backend instead."
        )
        axes = []
        for i in range(3):
            if i != axis:
        ...
        return scipy.ndimage.rotate(
            arr, -anp.rad2deg(theta), reshape=False, axes=axes, mode="nearest", order=1
        )
    elif backend == "pytorch":
        try:
            theta = theta.view(1)
        ...
        axis_arrangement = [0, 1, 2, 3]
        # Move channel to the 2nd dimension.
        axis_arrangement[1], axis_arrangement[3] = (
            axis_arrangement[3],
            axis_arrangement[1],
        )
        # Move invariant axis to front.
        ...
        arr = permute_axes(arr, axis_arrangement, override_backend="pytorch")
        naught = cast(
            tc.tensor([0.0], device=device),
            pytorch_dtype_query_mapping_dict[theta.dtype],
            override_backend="pytorch",
        )
        ...
        m = cast(
            tile(m, [arr.shape[0], 1, 1], override_backend="pytorch"),
            pytorch_dtype_query_mapping_dict[arr.dtype],
            override_backend="pytorch",
        )
        ...
        arr = tc.nn.functional.grid_sample(
            arr, g, padding_mode="border", align_corners=False
        )
        arr = permute_axes(
            arr,
            [
