...
class LargeArray(object):
    ...
    def create_file_object(self, fname, use_checkpoint=False):
        ...
        try:
            self.f = h5py.File(
                os.path.join(self.output_folder, fname), fmode, driver="mpio", comm=comm
            )
        except:
    ...
    def read_chunks_from_distributed_object(
        ...
        obj = get_subblocks_from_distributed_object_mpi(
            a,
            self.slice_catalog,
            probe_pos,
            this_ind_batch_allranks,
    ...
    def reverse_rotate_data_in_file(
        self, coords, interpolation="bilinear", precalculate_rotation_coords=True
    ):
        revert_rotation_to_hdf5(
            self.dset,
            coords,
            rank,
            n_ranks,
    ...
    def rotate_array(
        ...
        if precalculate_rotation_coords:
            ...
            b = sp_rotate(
                a, -coords, axes=(1, 2), reshape=False, order=1, mode="nearest"
            )
        ...
        if dtype is not None:
            if overwrite_arr:
                if self.distribution_mode == "distributed_object":
                    ...
                    self.arr = w.cast(
                        self.arr, dtype, override_backend=override_backend
                    )
            else:
                if self.distribution_mode == "distributed_object":
                    ...
                    self.arr_rot = w.cast(
                        self.arr_rot, dtype, override_backend=override_backend
                    )
    ...
    def write_chunks_to_file(
        ...
        if arr_channel_1 is not None:
            arr_channel_1 = w.to_numpy(arr_channel_1)
        if write_difference:
            if self.monochannel:
    ...
    def sync_chunks_to_distributed_object(
        ...
        obj = np.array(obj)
        self.arr = sync_subblocks_among_distributed_object_mpi(
            obj,
            self.arr,
            self.slice_catalog,
            probe_pos,
...
class ObjectFunction(LargeArray):
    ...
    def create_temporary_file_object(self):
        ...
        """
        try:
            self.f_rot = h5py.File(
                os.path.join(self.output_folder, "intermediate_obj_rot.h5"),
                "w",
                driver="mpio",
                comm=comm,
            )
            ...
            self.f_rot = h5py.File(
                os.path.join(self.output_folder, "intermediate_obj_rot.h5"), "w"
            )
        self.dset_rot = self.f_rot.create_dataset(
            "obj", shape=self.full_size, dtype="float64"
        )
    ...
    def initialize_distributed_array(
        ...
        if self.slice_catalog[rank] is not None:
            delta, beta = initialize_object_for_do(
                self.full_size[:-1],
                slice_catalog=self.slice_catalog,
                ds_level=self.ds_level,
                object_type=self.object_type,
                initial_guess=initial_guess,
                output_folder=self.output_folder,
    ...
    def apply_finite_support_mask_to_array(
    ...
    def update_using_external_algorithm(self, algorithm, kwargs, device=None):
        if algorithm == "ctf":
            ...
            phase = c.multidistance_ctf_wrapped(
                this_prj_batch,
                free_prop_cm,
                energy_ev,
                psize_cm,
...
class Mask(LargeArray):
    def __init__(
        ...
        super(Mask, self).__init__(
            full_size, distribution_mode, monochannel=True, output_folder=output_folder
        )
        self.mask = None
        self.ds_level = ds_level
    ...
    def create_file_object(self, use_checkpoint=False):
        super(Mask, self).create_file_object(
            "intermediate_mask.h5", use_checkpoint=use_checkpoint
        )
