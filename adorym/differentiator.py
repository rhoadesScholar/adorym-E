...
class Differentiator(object):
    ...
    def get_gradients(self, **kwargs):
        ...
        """
        gradients = w.get_gradients(
            self.loss_object, opt_args_ls=self.opt_args_ls, **kwargs
        )
        return gradients

    def get_l_h_hessian_and_h_x_jacobian_mvps(
        self, forward_model, ind_opt_arg, **kwargs
    ):
        """
        ...
        self.func_vjp, _ = w.vjp(forward_model.predict, [ind_opt_arg])(
            *list(kwargs.values())
        )
        self.func_jvp = w.jvp(forward_model.predict, [ind_opt_arg])(
            *list(kwargs.values())
        )
        ...
        this_prj_batch = forward_model.get_data(
            kwargs["this_i_theta"],
            kwargs["this_ind_batch"],
            theta_downsample=forward_model.common_vars["theta_downsample"],
            ds_level=forward_model.common_vars["ds_level"],
        )
        self.func_hvp, self.jloss = w.hvp(forward_model.loss, [0])(
            this_pred_batch, this_prj_batch, obj
        )
