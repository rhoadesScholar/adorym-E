...
class ForwardModel(object):
    ...
    def get_regularization_value(self, obj, device=None):
        reg = w.create_variable(0.0, device=self.device)
        for r in self.reg_list:
            reg = reg + r.get_value(
                obj, distribution_mode=self.distribution_mode, device=device
            )
        print_flush(
            "  Reg term = {}.".format(w.to_numpy(reg)), 0, rank, **self.stdout_options
        )
        return reg
        ...
        if self.loss_function_type == "lsq":
            ...
            if self.raw_data_type == "magnitude":
                loss = w.mean(
                    ...
                    - w.abs(this_prj_batch) ** 2
                    * self.poisson_multiplier
                    * w.log(this_pred_batch**2 * self.poisson_multiplier)
                )
            elif self.raw_data_type == "intensity":
                loss = w.mean(
                    this_pred_batch**2 * self.poisson_multiplier
                    - w.abs(this_prj_batch)
                    * self.poisson_multiplier
                    * w.log(this_pred_batch**2 * self.poisson_multiplier)
                )
        return loss
    ...
    def get_data(self, this_i_theta, this_ind_batch, theta_downsample=None, ds_level=1):
        if theta_downsample is None:
            theta_downsample = 1
        this_prj_batch = self.prj[this_i_theta * theta_downsample, this_ind_batch]
        this_prj_batch = w.create_variable(
            abs(this_prj_batch), requires_grad=False, device=self.device
        )
        if ds_level > 1:
    ...
    def loss(self, this_pred_batch, this_prj_batch, obj):
        ...
        if beamstop is not None:
            ...
            this_pred_batch = w.reshape(
                this_pred_batch[beamstop_mask_stack], [beamstop_mask_stack.shape[0], -1]
            )
            this_prj_batch = w.reshape(
                this_prj_batch[beamstop_mask_stack], [beamstop_mask_stack.shape[0], -1]
            )
            print_flush(
                "  {} valid pixels remain after applying beamstop mask.".format(
                    this_pred_batch.shape[1]
                ),
                0,
                rank,
            )
    ...
    def get_loss_function(self):
        ...
        def calculate_loss(obj, this_i_theta, this_ind_batch, *args, **kwargs):
            ...
            this_prj_batch = self.get_data(
                this_i_theta,
                this_ind_batch,
                theta_downsample=theta_downsample,
                ds_level=ds_level,
            )
...
class PtychographyModel(ForwardModel):
    def __init__(
        ...
        super(PtychographyModel, self).__init__(
            loss_function_type,
            distribution_mode,
            device,
            common_vars_dict,
    ...
    def predict(
        ...
        if optimize_probe_defocusing:
            ...
            probe_real, probe_imag = w.convolve_with_transfer_function(
                probe_real, probe_imag, h_probe_real, h_probe_imag
            )
        ...
        if optimize_probe_pos_offset:
            ...
            probe_real, probe_imag = realign_image_fourier(
                probe_real, probe_imag, this_offset, axes=(1, 2), device=device_obj
            )
        ...
        if not two_d_mode and not self.distribution_mode:
            if not optimize_tilt and self.common_vars["initial_tilt"] is None:
                ...
                obj_rot = w.rotate(
                    obj, tilt_ls[0, this_i_theta], axis=0, device=device_obj
                )
                obj_rot = w.rotate(
                    obj_rot, tilt_ls[1, this_i_theta], axis=1, device=device_obj
                )
                obj_rot = w.rotate(
                    obj_rot, tilt_ls[2, this_i_theta], axis=2, device=device_obj
                )

        else:
            if optimize_tilt:
                raise NotImplementedError(
                    "Tilt optimization is not impemented for distributed mode or when two_d_mode is on."
                )
            obj_rot = obj
        ...
        for k, pos_batch in enumerate(probe_pos_batch_ls):
            ...
            for j in range(len(pos_batch)):
                if (
                    ...
                    or len(w.nonzero(probe_pos_correction > 1e-3)) > 0
                ):
                    this_shift = probe_pos_correction[
                        this_i_theta, this_ind_batch[k * n_dp_batch + j]
                    ]
            ...
            if self.distribution_mode is None:
                if len(pos_batch) == 1:
                    ...
                    if (
                        pos_y == 0
                        and pos_x == 0
                        and probe_size[0] == this_obj_size[0]
                        and probe_size[1] == this_obj_size[1]
                    ):
                        subobj = obj_rot
                    else:
                        subobj = obj_rot[
                            pos_y : pos_y + probe_size[0],
                            pos_x : pos_x + probe_size[1],
                            :,
                            :,
                        ]
            ...
            if n_probe_modes == 1:
                ...
                ex_real, ex_imag = multislice_propagate_batch(
                    ...
                    is_minus_logged=self.is_minus_logged,
                    pure_projection_return_sqrt=flag_pp_sqrt,
                    shift_exit_wave=this_prj_offset,
                )
        ...
        if rank == 0 and debug and self.i_call % 10 == 0:
            ...
            dxchange.write_tiff(
                ex_mag_val,
                os.path.join(output_folder, "intermediate", "detected_mag"),
                dtype="float32",
                overwrite=True,
            )
        self.i_call += 1
...
class SingleBatchFullfieldModel(PtychographyModel):
    ...
    def __init__(
        ...
        super(SingleBatchFullfieldModel, self).__init__(
            loss_function_type,
            distribution_mode,
            device,
            common_vars_dict,
        ...
        if not two_d_mode and not self.distribution_mode:
            if not self.rotate_out_of_loop:
                if precalculate_rotation_coords:
                    ...
                    obj_rot = rotate_no_grad(
                        obj, theta_ls[this_i_theta], axis=0, device=device_obj
                    )
            else:
                obj_rot = obj
        else:
...
class SingleBatchPtychographyModel(PtychographyModel):
    ...
    def __init__(
        ...
        super(SingleBatchPtychographyModel, self).__init__(
            loss_function_type,
            distribution_mode,
            device,
            common_vars_dict,
    ...
    def predict(
        ...
        if self.distribution_mode is None:
            ...
            pos_x = pos[1] + pad_arr[1, 0]
            obj_rot = obj_rot[
                pos_y : pos_y + probe_size[0], pos_x : pos_x + probe_size[1], :, :
            ]
            obj_rot = w.reshape(obj_rot, [1, *obj_rot.shape])
...
class SparseMultisliceModel(ForwardModel):
    def __init__(
        ...
        super(SparseMultisliceModel, self).__init__(
            loss_function_type,
            distribution_mode,
            device,
            common_vars_dict,
        ...
        if not two_d_mode and not self.distribution_mode:
            if not self.rotate_out_of_loop:
                if precalculate_rotation_coords:
                    ...
                    obj_rot = rotate_no_grad(
                        obj, theta_ls[this_i_theta], device=device_obj
                    )
            else:
                obj_rot = obj
        else:
        ...
        for k, pos_batch in enumerate(probe_pos_batch_ls):
            ...
            if n_probe_modes == 1:
                if len(probe_real_ls.shape) == 3:
                    ...
                    this_probe_real_ls = probe_real_ls[:, 0, :, :]
                    this_probe_imag_ls = probe_imag_ls[:, 0, :, :]
                ex_real, ex_imag = sparse_multislice_propagate_batch(
                    u,
                    v,
                    subobj_ls,
                    this_probe_real_ls,
...
class MultiDistModel(ForwardModel):
    def __init__(
        ...
        super(MultiDistModel, self).__init__(
            loss_function_type,
            distribution_mode,
            device,
            common_vars_dict,
    ...
    def predict(
        ...
        if not self.distribution_mode:
            obj_rot, pad_arr = pad_object(
                obj_rot,
                this_obj_size,
                this_pos_batch - szw_arr,
                subprobe_size + 2 * szw_arr,
                unknown_type=unknown_type,
            )
        ...
        if safe_zone_width > 0:
            ...
            probe_real_sz = w.pad(
                probe_real, [[0, 0]] + pad_arr, mode="constant", constant_values=1
            )
            probe_imag_sz = w.pad(
                probe_imag, [[0, 0]] + pad_arr, mode="constant", constant_values=0
            )
        else:
            ...
            if self.distribution_mode is None:
                if n_blocks > 1:
                    ...
                    if len(pos_batch) == 1:
                        ...
                        sub_probe_real = probe_real_sz[
                            :,
                            pos_y : pos_y + subprobe_size[0] + safe_zone_width * 2,
                            pos_x : pos_x + subprobe_size[1] + safe_zone_width * 2,
                        ]
                        sub_probe_imag = probe_imag_sz[
                            :,
                            pos_y : pos_y + subprobe_size[0] + safe_zone_width * 2,
                            pos_x : pos_x + subprobe_size[1] + safe_zone_width * 2,
                        ]
                        subprobe_subbatch_real_ls = w.reshape(
                            sub_probe_real, [1, *sub_probe_real.shape]
                        )
                        subprobe_subbatch_imag_ls = w.reshape(
                            sub_probe_imag, [1, *sub_probe_imag.shape]
                        )
                    ...
                    subprobe_subbatch_real_ls = w.reshape(
                        probe_real_sz, [1, *probe_real_sz.shape]
                    )
                    subprobe_subbatch_imag_ls = w.reshape(
                        probe_imag_sz, [1, *probe_imag_sz.shape]
                    )
            ...
            for k, pos_batch in enumerate(probe_pos_batch_ls):
                for i_mode in range(n_probe_modes):
                    if self.forward_algorithm == "fresnel":
                        temp_real, temp_imag = multislice_propagate_batch(
                            subobj_ls_ls[k],
                            subprobe_real_ls_ls[k][:, i_mode, :, :],
                            subprobe_imag_ls_ls[k][i_mode, :, :],
                            energy_ev,
                            psize_cm * ds_level,
                            kernel=h,
                            free_prop_cm=this_dist,
                            binning=self.binning,
                            obj_batch_shape=[
                                len(pos_batch),
                                subprobe_size[0] + 2 * safe_zone_width,
                                subprobe_size[1] + 2 * safe_zone_width,
                                this_obj_size[-1],
                            ],
                            fresnel_approx=fresnel_approx,
                            pure_projection=pure_projection,
                            ...
                            type=unknown_type,
                            sign_convention=self.sign_convention,
                            optimize_free_prop=optimize_free_prop,
                            u_free=u_free,
                            v_free=v_free,
                            scale_ri_by_k=self.scale_ri_by_k,
                            kappa=kappa,
                        ...
                        temp_real, temp_imag = modulate_and_get_ctf(
                            subobj_ls_ls[k],
                            energy_ev,
                            this_dist,
                            u_free,
                            v_free,
                            kappa=10 ** ctf_lg_kappa[0],
                        )
                    else:
        ...
        if safe_zone_width > 0:
            ex_mag_ls = ex_mag_ls[
                :,
                safe_zone_width : safe_zone_width + subprobe_size[0],
    ...
    def get_loss_function(self):
        def calculate_loss(
            ...
            for i in range(1, n_dists):
                this_ind_batch_full = np.concatenate(
                    [this_ind_batch_full, this_ind_batch + i * n_blocks]
                )
            ...
            if optimize_prj_affine:
                ...
                for i in range(n_dists):
                    this_prj_batch_idist = this_prj_batch[
                        len(this_ind_batch) * i : len(this_ind_batch) * (i + 1)
                    ]
                    this_prj_batch_idist = w.affine_transform(
                        this_prj_batch_idist, prj_affine_ls[i]
                    )
            ...
            if optimize_probe_pos_offset:
                this_offset = probe_pos_offset[this_i_theta]
                this_prj_batch, _ = realign_image_fourier(
                    this_prj_batch,
                    w.zeros_like(this_prj_batch),
                    this_offset,
                    axes=(0, 1),
                    device=self.device,
                )
            ...
            if optimize_all_probe_pos:
                ...
                for i in range(n_dists):
                    ...
                    this_prj_batch_idist, _ = realign_image_fourier(
                        this_prj_batch_idist,
                        w.zeros_like(this_prj_batch_idist),
                        this_shift,
                        axes=(1, 2),
                        device=self.device,
                    )
