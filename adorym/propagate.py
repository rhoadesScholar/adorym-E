...
def get_kernel(
    dist_nm, lmbda_nm, voxel_nm, grid_shape, fresnel_approx=True, sign_convention=1
):
    ...
    if fresnel_approx:
        ...
        quad_mask = quad > 0
        H = np.exp(
            sign_convention * 1j * 2 * PI * dist_nm / lmbda_nm * np.sqrt(quad_inner)
        )
        H = H * quad_mask
    return H


def get_kernel_wrapped(
    u,
    v,
    dist_nm,
    lmbda_nm,
    ...
    if fresnel_approx:
        # Use sign_convention = 1 for Goodman convention: exp(ikz); n = 1 - delta + i * beta
        # Use sign_convention = -1 for opposite convention: exp(-ikz); n = 1 - delta - i * beta
        h_real, h_imag = w.exp_complex(
            0.0, -sign_convention * PI * lmbda_nm * dist_nm * (u**2 + v**2)
        )
    else:
        # Use sign_convention = 1 for Goodman convention: exp(ikz); n = 1 - delta + i * beta
        # Use sign_convention = -1 for opposite convention: exp(-ikz); n = 1 - delta - i * beta
        ...
        quad_mask = quad > 0
        h_real, h_imag = w.exp_complex(
            0.0, sign_convention * 2 * PI * dist_nm / lmbda_nm * np.sqrt(quad_inner)
        )
...
def get_kernel_ir(dist_nm, lmbda_nm, voxel_nm, grid_shape, sign_convention=1):
    ...
    k = 2 * PI / lmbda_nm
    ymin, xmin = np.array(size_nm)[:2] / -2.0
    dy, dx = voxel_nm[0:2]
    x = np.arange(xmin, xmin + size_nm[1], dx)
    y = np.arange(ymin, ymin + size_nm[0], dy)
    x, y = np.meshgrid(x, y)
    h = (
        np.exp(sign_convention * 1j * k * dist_nm)
        / (1j * lmbda_nm * dist_nm)
        * np.exp(sign_convention * 1j * k / (2 * dist_nm) * (x**2 + y**2))
    )
    H = np.fft.fft2(h)
...
def multislice_propagate_batch(
    ...
    if delta_cm is not None:
        voxel_nm = np.array([psize_cm, psize_cm, delta_cm]) * 1.0e7
    else:
        voxel_nm = np.array([psize_cm] * 3) * 1.0e7

    lmbda_nm = 1240.0 / energy_ev
    mean_voxel_nm = np.prod(voxel_nm) ** (1.0 / 3)
    ...
    if pure_projection:
        k1 = 2.0 * PI * delta_nm / lmbda_nm if scale_ri_by_k else 1.0
        if type == "delta_beta":
            # Use sign_convention = 1 for Goodman convention: exp(ikz); n = 1 - delta + i * beta
            # Use sign_convention = -1 for opposite convention: exp(-ikz); n = 1 - delta - i * beta
            ...
            if is_minus_logged:
                if pure_projection_return_sqrt:
                    c_real, c_imag = (
                        w.sqrt(-w.log(c_real**2 + c_imag**2) + 1e-10),
                        0,
                    )
                else:
        ...
        probe_real, probe_imag = (
            probe_real * c_real - probe_imag * c_imag,
        ...
        for i_step in range(n_steps):
            ...
            if repeating_slice is None:
                delta_slice = (
                    grid_batch[:, :, :, i_slice : i_slice + this_step, 0]
                    if this_step > 1
                    else grid_batch[:, :, :, i_slice, 0]
                )
            else:
            ...
            if kappa is not None:
                ...
                if repeating_slice is None:
                    beta_slice = (
                        grid_batch[:, :, :, i_slice : i_slice + this_step, 1]
                        if this_step > 1
                        else grid_batch[:, :, :, i_slice, 1]
                    )
                else:
    ...
    if shift_exit_wave is not None:
        probe_real, probe_imag = realign_image_fourier(
            probe_real, probe_imag, shift_exit_wave, axes=(1, 2), device=device
        )
        ...
        if isinstance(free_prop_cm, str) and free_prop_cm == "inf":
            ...
            if sign_convention == 1:
                probe_real, probe_imag = w.fft2_and_shift(
                    probe_real, probe_imag, axes=[1, 2], normalize=normalize_fft
                )
            else:
...
def multislice_backpropagate_batch(
    grid_batch,
    probe_real,
    probe_imag,
    energy_ev,
...
def modulate_and_get_ctf(
    grid_batch, energy_ev, free_prop_cm, u_free=None, v_free=None, kappa=50.0
):
    ...
    delta_slice = p[:, :, :, 0]
    beta_slice = p[:, :, :, 1]
    probe_real, probe_imag = pure_phase_ctf(
        u_free, v_free, delta_slice, beta_slice, dist_nm, lmbda_nm, kappa=kappa
    )
    return probe_real, probe_imag


def sparse_multislice_propagate_batch(
    u,
    v,
    grid_batch,
    probe_real,
    ...
    for i in range(n_slices):
        ...
        if i < n_slices - 1:
            ...
            probe_real, probe_imag = fresnel_propagate_wrapped(
                ...
                probe_real,
                probe_imag,
                slice_pos_nm_ls[i + 1] - slice_pos_nm_ls[i],
                lmbda_nm,
                voxel_nm,
                device=device,
                sign_convention=sign_convention,
...
def fresnel_propagate(
    ...
    if h is None:
        h = get_kernel(
            dist_nm, lmbda_nm, voxel_nm, grid_shape, sign_convention=sign_convention
        )
    h_real = np.real(h)
    h_imag = np.imag(h)
    h_real = w.create_variable(
        h_real, requires_grad=False, device=device, override_backend=override_backend
    )
    h_imag = w.create_variable(
        h_imag, requires_grad=False, device=device, override_backend=override_backend
    )
    probe_real, probe_imag = w.convolve_with_transfer_function(
        probe_real, probe_imag, h_real, h_imag, override_backend=override_backend
    )
    return probe_real, probe_imag


def fresnel_propagate_wrapped(
    u,
    v,
    probe_real,
    probe_imag,
    ...
    if len(probe_real.shape) == 3:
        ...
        grid_shape = probe_real.shape
    if h is None:
        h_real, h_imag = get_kernel_wrapped(
            u,
            v,
            dist_nm,
            lmbda_nm,
...
def ctf(
    ...
    if len(probe_real.shape) == 3:
        ...
        grid_shape = probe_real.shape
    probe_real, probe_imag = w.fft2(
        probe_real, probe_imag, override_backend=override_backend, normalize=True
    )
    if h is None:
        h_real, h_imag = get_kernel_wrapped(
            u,
    ...
    probe_real, probe_imag = w.convolve_with_impulse_response(
        a1_real,
        a1_imag,
        a2_real,
        a2_imag,
...
def pure_phase_ctf(
    ...
    probe_real, probe_imag = w.fft2(
        delta_slice,
        w.zeros_like(delta_slice, requires_grad=False),
        override_backend=override_backend,
    )
    xi = PI * lmbda_nm * dist_nm * (u**2 + v**2)
    osc = 2 * (w.sin(xi) + 1.0 / kappa * w.cos(xi))
    probe_real = osc * probe_real
    probe_imag = osc * probe_imag
    probe_real, probe_imag = w.ifft2(
        probe_real, probe_imag, override_backend=override_backend
    )
