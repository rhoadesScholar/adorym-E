...
def create_summary(save_path, locals_dict, var_list=None, preset=None, verbose=True):
    ...
    for var_name in var_list:
        if var_name == "learning_rate" and locals_dict["optimizer"] is not None:
            ...
            "learning_rate" in var_name
            and locals_dict["optimizer_" + var_name[:-14]] is not None
        ):
            continue
        else:
            try:
...
def save_checkpoint(
    ...
    np.savetxt(
        os.path.join(path, "checkpoint.txt"), np.array([i_epoch, i_batch]), fmt="%d"
    )
    if distribution_mode is None:
        ...
        if obj_array is not None:
            np.save(
                os.path.join(path, "obj_checkpoint_rank_{}.npy".format(rank)), obj_array
            )
        if optimizer is not None:
...
def restore_checkpoint(
    output_folder, distribution_mode=None, optimizer=None, dtype="float32"
):

    path = os.path.join(output_folder, "checkpoint")
    i_epoch, i_batch = [
        int(i) for i in np.loadtxt(os.path.join(path, "checkpoint.txt"))
    ]
    if distribution_mode is None:
...
def print_flush(
    ...
    **kwargs
    ...
    if (designate_rank is None or this_rank == designate_rank) and save_stdout:
        try:
            f = open(
                os.path.join(output_folder, "stdout_{}.txt".format(timestamp)), "a"
            )
        except:
