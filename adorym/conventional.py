...
def alt_reconstruction_epie(
    ...
    **kwargs
    ...
    with w.no_grad():
        ...
        # Pad if needed
        obj_stack, pad_arr = pad_object(
            obj_stack, this_obj_size, probe_pos, probe_size, unknown_type="real_imag"
        )
        ...
        for i_epoch in range(n_epochs):
            for j in range(len(probe_pos)):
                ...
                if i_batch < minibatch_size and i_batch < prj.shape[1]:
                    ...
                    this_prj_batch = prj[
                        0, j * minibatch_size : j * minibatch_size + i_batch, :, :
                    ]
                    this_prj_batch = w.create_variable(
                        this_prj_batch, requires_grad=False, device=device_obj
                    )
                    ...
                    ex_real_ls, ex_imag_ls = (
                        probe_real_ls * c_real - probe_imag_ls * c_imag,
                        probe_real_ls * c_imag + probe_imag_ls * c_real,
                    )
                    ...
                    o_up_real = (
                        probe_real_ls * d_real_ls + probe_imag_ls * d_imag_ls
                    ) / norm
                    o_up_imag = (
                        probe_real_ls * d_imag_ls - probe_imag_ls * d_real_ls
                    ) / norm
                    ...
                    norm = w.max(
                        subobj_ls[:, :, :, 0, 0] ** 2 + subobj_ls[:, :, :, 0, 1] ** 2
                    )
                    p_up_real = (
                        subobj_ls[:, :, :, 0, 0] * d_real_ls
                        + subobj_ls[:, :, :, 0, 1] * d_imag_ls
                    ) / norm
                    p_up_imag = (
                        subobj_ls[:, :, :, 0, 0] * d_imag_ls
                        - subobj_ls[:, :, :, 0, 1] * d_real_ls
                    ) / norm
                    ...
                    for i, k in enumerate(
                        range(j * minibatch_size, j * minibatch_size + i_batch)
                    ):
                        pos = probe_pos[k]
                        pos[0] = pos[0] + pad_arr[0, 0]
                        pos[1] = pos[1] + pad_arr[1, 0]
...
def multidistance_ctf_wrapped(
    ...
    this_prj_batch = w.create_variable(
        this_prj_batch, requires_grad=False, device=device
    )
    if prj_affine_ls is not None:
        for i in range(len(prj_affine_ls)):
            this_prj_batch[i] = w.affine_transform(
                this_prj_batch[i : i + 1], prj_affine_ls[i]
            )
    if safe_zone_width > 0:
        this_prj_batch = w.pad(
            this_prj_batch,
            [
                (0, 0),
    ...
    this_prj_batch_ft_r, this_prj_batch_ft_i = w.fft2(
        this_prj_batch - 1,
    ...
    for i in range(len(dist_nm_ls)):
        ...
        prj_real_ls.append(
            (w.sin(xi) + 1.0 / kappa * w.cos(xi)) * this_prj_batch_ft_r[i]
        )
        prj_imag_ls.append(
            (w.sin(xi) + 1.0 / kappa * w.cos(xi)) * this_prj_batch_ft_i[i]
        )
    ...
    for i in range(len(dist_nm_ls)):
        xi = PI * lmbda_nm * dist_nm_ls[i] * (u_free**2 + v_free**2)
        osc_ls.append(2 * (w.sin(xi) + 1.0 / kappa * w.cos(xi)) ** 2)
    ...
    return phase[
        safe_zone_width : phase.shape[0] - safe_zone_width,
