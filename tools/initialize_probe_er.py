...
parser.add_argument(
    ...
    default=0.8,
    type=float,
    help="Weight of magnitude reduction outside finite support.",
)
parser.add_argument(
    "--mask_radius",
    default=64,
    type=int,
    help="Radius of finite support mask to be used for ER probe retrieval.",
)
parser.add_argument(
    "--normalize",
    default=0,
    type=bool,
    help="Whether to divide diffraction pattern intensity by probe size.",
)
parser.add_argument(
    "--raw_data_type",
    default="intensity",
    type=str,
    help="Whether raw data is intensity or magnitude.",
)
...
probe = np.random.normal(1, 0.1, probe_shape) + np.exp(
    1j * np.random.normal(0, 0.1, probe_shape)
)
probe *= mask
...
dxchange.write_tiff(
    np.angle(probe), "guessted_probe_phase", dtype="float32", overwrite=True
)
dxchange.write_tiff(abs(F) ** 2, "guessted_probe_dp", dtype="float32", overwrite=True)
dxchange.write_tiff(
    img**2, "guessted_probe_dp_measured", dtype="float32", overwrite=True
)
