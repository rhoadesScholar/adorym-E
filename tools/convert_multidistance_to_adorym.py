...
parser.add_argument(
    "distances_cm",
    default="None",
    help="Distances in cm, separated by comma (no spaces allowed)",
)
...
parser.add_argument(
    "--n_blocks_y", default=1, type=int, help="Number of subblocks in y."
)
parser.add_argument(
    "--n_blocks_x", default=1, type=int, help="Number of subblocks in x."
)
parser.add_argument(
    "--energy_ev", default=5000.0, type=float, help="Beam energy in ev."
)
parser.add_argument(
    "--psize_cm", default=1e-4, type=float, help="Sample plane pixel size in cm."
)
...
if n_blocks == 1:
    ...
    block_range_ls = adorym.get_subdividing_params(
        raw_img_shape, n_blocks_y, n_blocks_x
    )
...
dset = grp.create_dataset(
    "data",
    shape=[n_theta, n_blocks * n_dists, block_size_y, block_size_x],
    dtype="float32",
)
...
for i_theta in range(n_theta):
    for i_dist in range(n_dists):
        ...
        if n_blocks == 1:
            ...
            block_ls = adorym.subdivide_image(
                img, block_range_ls, override_backend="numpy"
            )
